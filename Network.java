    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    public String recommendWhoToFollow(String name) {
        ...
        for (int i = 0; i < userCount; i++) {
            ...
            int mutualWithThisOne = getUser(name).countMutual(users[i]);
            if (mutualWithThisOne > maxMutual) {
                mostRecommendedUserToFollow = users[i].getName();
                maxMutual = mutualWithThisOne;
            }
        }
    ...
    public String mostPopularUser() {
        ...
        int maxFollowers = 0;
        for (int i = 0; i < userCount; i++) {
            int FollowersThisOne = followeeCount(users[i].getName());
            if (FollowersThisOne > maxFollowers) {
                mostPopularUserName = users[i].getName();;
                maxFollowers = FollowersThisOne;
            }
        }
